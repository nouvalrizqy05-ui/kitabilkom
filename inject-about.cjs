...
const aboutSection = `
      {/* ==================== ABOUT KITAB ILKOM ==================== */}
      <section className="about-custom" style={{ position: 'relative', padding: '6rem 0', backgroundColor: 'var(--bg-surface)', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: 0.05, zIndex: 0 }}>
          <img src="/assets/gedung.png" alt="Background" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={(e) => { e.target.style.display = 'none'; }} />
        </div>
...
if (code.includes(target)) {
    code = code.replace(target, aboutSection + target);
    fs.writeFileSync(path, code, 'utf8');
    console.log('About section added successfully.');
} else {
    console.log('Target not found in Home.jsx.');
}
